...
do {
printf("\nEnter the number of resources: ");scanf("%d", &nResources);
...
if (!safe_sequence) {printf("Failed to allocate memory to safe_sequence, exiting program...\n");
exit(-1);
}
// set default values
for(i = 0; i < nProcesses; ++i) {
...
for(i = 0; i < nProcesses; ++i) {
pthread_create(&processes[i], &attr, display_result,
...
printf("\n\tAllocated: ");
for(i = 0; i < nResources; ++i) {printf("%3d", allocated[p][i]);
}
printf("\n\tNeeded: ");
for(i = 0; i < nResources; ++i) {
printf("%3d", need[p][i]);
